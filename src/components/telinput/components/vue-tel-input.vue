...
export default {
  ...
  computed: {
    ...
    phoneObject () {
      ...
      let valid = result.isValid ? result.isValid() : undefined
      let formatted = this.phone

      if (valid) {
        formatted = result.format
          ? result.format(this.parsedMode.toUpperCase())
          : undefined
      }
  ...
  watch: {
    ...
    activeCountry (value, oldValue) {
      if (!value && oldValue && oldValue.iso2) {
  ...
  methods: {
    ...
    choose (country) {
      ...
      localStorage.setItem(
        'contry_dialcode_phone',
        country.dialCode ? country.dialCode : 966
      )
      if (
        (country.dialCode && country.dialCode == '966') ||
        !country.dialCode
      ) {
        this.minnumbers = countries.SA.min
        this.maxnumbers = countries.SA.max
      } else if (country.dialCode && country.dialCode == '91') {
        this.minnumbers = countries.IN.min
        this.maxnumbers = countries.IN.max
      } else if (country.dialCode && country.dialCode == '961') {
        this.minnumbers = countries.LEB.min
        this.maxnumbers = countries.LEB.max
      } else if (country.dialCode && country.dialCode == '964') {
        this.minnumbers = countries.IR.min
        this.maxnumbers = countries.IR.max
      } else {
        this.minnumbers = countries.DEFAULT.min
        this.maxnumbers = countries.DEFAULT.max
      }
