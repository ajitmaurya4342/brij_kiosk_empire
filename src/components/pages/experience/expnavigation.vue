<template>
  <div>
    ...
    <section class="section-banner simple-banner">
      <slick
        v-if="getExperiencesBanners.length > 0"
        ref="bannerSlider"
        :options="bannerSlider"
      >
        <div
          v-for="(data, index) in getExperiencesBanners"
          :key="index"
        ...
        >
          <div class="banner-img">
            <div>
              <img
                :src="checkBaseUrl(data.movie_experience_image_banner_url)"
                class="image"
              />
            </div>
          </div>
    ...
    >
      ...
      >
        <ul class="list-inline my-0 py-0">
          ...
          >
            <a
              :style="expSelected == experience.ID ? 'color:red' : ''"
...
export default {
  ...
  computed: {
    ...
    getExperiencesBanners () {
      return this.$store.getters.getExperiences.filter(b => !!b.movie_experience_image_banner_url)
    },
  },
  watch: {
    '$route.params': {
      handler: function (newVal, oldVal) {
        console.log('$router.params', { newVal, oldVal })
      },
      deep: true,
    },
  },
  ...
  methods: {
    onShowExperience (data) {
      ...
      this.expSelected = data.ID
      this.selectedExpName = data.name
      console.log({ onShowExperience: data })

      this.$emit('changeHeaderExp', data.ID)
      ...
      if (data) {
        new Promise(resolve => {
          ...
          .finally(() => {
            console.log({ onShowExperienceChangeHeader: data })
            this.hideLoading()
          })
      }
    },
